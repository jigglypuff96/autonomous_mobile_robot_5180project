...
[cmdV,cmdW] = limitCmds(fwdvel,angvel,0.15,0.2);
SetFwdVelAngVelCreate(obj,cmdV,cmdW)

set(handles.text_fwdvel,'String',sprintf('%1.2f',cmdV))
set(handles.text_angvel,'String',sprintf('%1.2f',cmdW))

if cmdV ~= fwdvel || cmdW ~= angvel
    set(handles.txtLimits,'Visible','on')
else
    set(handles.txtLimits,'Visible','off')   
end
guidata(hObject, handles);
